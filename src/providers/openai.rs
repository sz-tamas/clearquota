...
async fn fetch_completions_usage(
	...
	for page_count in 0..100 {
		...
		let parsed = parse_usage_entries(&payload).map_err(|_| {
			eprintln!("event=openai_usage_response_invalid endpoint=/organization/usage/completions");
			ProviderError::OpenAiUsageInvalidResponse
		})?;
		entries.extend(parsed);
		let has_more = payload.get("has_more").and_then(Value::as_bool).unwrap_or(false);
		page = payload.get("next_page").and_then(Value::as_str).map(ToOwned::to_owned);
			...
			bucket
				...
				.map(move |result| {
					Ok(OpenAiUsageLedgerEntry {
						...
						project_id: result.get("project_id").and_then(Value::as_str).map(ToOwned::to_owned),
						model: result.get("model").and_then(Value::as_str).map(ToOwned::to_owned),
					})
				})
		})
		.collect()
...
async fn fetch_costs(
	...
	for page_count in 0..100 {
		...
		let payload = get_json(client, secret, "https://api.openai.com/v1/organization/costs", &query).await?;
...
async fn fetch_spend_alerts(client: &reqwest::Client, secret: &SecretString) -> Result<f64, ProviderError> {
	let payload = get_json(
		client,
		secret,
...
async fn get_json(
	...
	let payload = response.json().await.map_err(|_| ProviderError::InvalidResponse)?;
	eprintln!(
				...
				.map(move |result| {
					...
					Ok(OpenAiCostLedgerEntry {
						...
						currency: amount
							...
							.to_owned(),
						project_id: result.get("project_id").and_then(Value::as_str).map(ToOwned::to_owned),
						api_key_id: result.get("api_key_id").and_then(Value::as_str).map(ToOwned::to_owned),
						line_item: result.get("line_item").and_then(Value::as_str).map(ToOwned::to_owned),
					})
				})
		})
		.collect()
...
mod tests {
	...
	fn sums_daily_costs() {
		...
		assert_eq!(costs.entries.iter().map(|entry| entry.amount).sum::<f64>(), 8.92);
	...
	fn creates_usage_metrics_from_period_totals() {
		...
		assert_eq!(
			metrics.iter().find(|metric| metric.id == "total_tokens").unwrap().used,
			1250.0
		);
		assert_eq!(
			metrics.iter().find(|metric| metric.id == "prompt_cache_hit_rate").unwrap().used,
			50.0
		);
		assert_eq!(
			metrics.iter().find(|metric| metric.id == "total_requests").unwrap().used,
			5.0
		);
	}
